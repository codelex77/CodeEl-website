...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    let query = supabase.from('codeel_testimonials').select('*').order('created_at', { ascending: false });
  ...
  if (req.method === 'PATCH') {
    ...
    const { error } = await supabase.from('codeel_testimonials').update({ approved }).eq('id', id);
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ success: true });
  }
  ...
  if (req.method === 'DELETE') {
    const { id } = req.body;
    if (!id) return res.status(400).json({ error: 'Missing id' });
    const { error } = await supabase.from('codeel_testimonials').delete().eq('id', id);
    if (error) return res.status(500).json({ error: error.message });
    return res.status(200).json({ success: true });
  }
